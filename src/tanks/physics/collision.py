...
class CollisionSystem:
    ...
    def check_bullet_wall_collision(self, bullet):
        ...
        """
        tiles_to_check = self._get_nearby_tiles(bullet.x, bullet.y, bullet.radius)
        
        for tile in tiles_to_check:
            hit, normal = self._check_bullet_tile_collision(bullet, tile)
            if hit:
                return True, normal
    ...
    def _check_bullet_tile_collision(self, bullet, tile_info):
        """Check bullet collision with tile and return normal if hit."""
        tx, ty, tile_type = tile_info
        edges = self._get_tile_edges(tx, ty, tile_type)
        ...
        for edge in edges:
            ...
            hit, point = line_circle_intersection(
                x1, y1, x2, y2, bullet.x, bullet.y, bullet.radius
            )
            ...
            if hit:
                ...
                if length > 0:
                    ...
                    to_bullet_x = bullet.x - x1
                    to_bullet_y = bullet.y - y1
    ...
    def _get_tile_edges(self, tx, ty, tile_type):
        ...
        from ..config.constants import (
            TILE_WALL_HORIZONTAL, TILE_WALL_VERTICAL,
            TILE_WALL_DIAGONAL_NE, TILE_WALL_DIAGONAL_NW,
            TILE_WALL_SOLID
        )
